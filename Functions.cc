...
/////////////////////////////////////////////////////////////////
bool recol(double & met, vector<PseudoJet> jets,vector<PseudoJet> leptons,vector<PseudoJet> neutrinos){
  ...
  if(1>0
   ...
   && met > MetMin
   //
   )return true; else return false; // close basic cuts
}// close recow
/////////////////////////////////////////////////////////////////
...
void isbtagged(vector<PseudoJet> jets, vector<int> & btag, vector<int> & bmistag, vector<int> & btrue){ 
  ...
  for (unsigned int i=0; i<jsize; i++) { // check wheter jet have inside a b's are taggable 
     ...
     if(seetruth>0) btrue.push_back(id); else btrue.push_back(0); // to mctruth
